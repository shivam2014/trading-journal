    ...
    try {
        ...
        for (const csvFile of csvFiles) {
            ...
            if (trades && trades.length > 0) {
                // Only add trades that haven't been processed yet
...
function updateDashboardStats() {
    const stats = calculateTradeMetrics(allTrades);
    
    // Update top stats bar
    document.getElementById('totalTrades').textContent = stats.totalTrades;
    document.getElementById('netPnL').textContent = formatCurrency(stats.netPnL);
    ...
    document.getElementById('winRateDetailed').textContent = `${(stats.winRate * 100).toFixed(1)}%`;
    document.getElementById('lossRate').textContent = `${(stats.lossRate * 100).toFixed(1)}%`;
...
function calculateTradeMetrics(trades) {
    const buyTrades = trades.filter(t => t.action.includes('buy'));
    const sellTrades = trades.filter(t => t.action.includes('sell'));
    
    // Basic metrics
    const totalBuyShares = buyTrades.reduce((sum, t) => sum + t.shares, 0);
    const totalBuyCost = buyTrades.reduce((sum, t) => sum + (t.shares * t.price), 0);
    ...
    const returnPercent = avgBuyPrice > 0 ? ((avgSellPrice - avgBuyPrice) / avgBuyPrice) * 100 : 0;
    ...
    // Calculate average holding period (weighted by trade value)
    const avgHoldingPeriod = totalTradeValue > 0 ? weightedHoldingDays / totalTradeValue : 0;
    
    // Calculate win rate
    const winRate = completedTrades > 0 ? (winningTrades / completedTrades) * 100 : 0;

    // Calculate R-multiple (risk-reward ratio)
    const winningTrade = trades.reduce((max, trade) => 
        trade.result > max.result ? trade : max, { result: -Infinity });
    const losingTrade = trades.reduce((min, trade) => 
        trade.result < min.result ? trade : min, { result: Infinity });
    ...
    return {
        ...
        rMultiple,
        volatility,
        maxWin: winningTrade.result,
        maxLoss: losingTrade.result
    };
}

// Update the trade group HTML generation
function generateTradeGroupHTML(group) {
    const metrics = calculateTradeMetrics(group.trades);
    const hasCompletedTrades = metrics.totalSellShares > 0;
                ...
                <div class="group-info">
                    ...
                    <div class="group-date-status">
                        ...
                        <span class="status-badge ${group.status.toLowerCase()}" role="status">
                            ${group.status}${group.status === 'PARTIALLY CLOSED' ? 
                ...
                <div class="group-summary" role="group" aria-label="Trade summary">
                    ...
                    <div class="summary-item">
                        ...
                        <span class="summary-value" aria-labelledby="holdtime-${group.id}">
                            ${metrics.avgHoldingPeriod.toFixed(1)}d
                        </span>
                    </div>
                </div>
            </div>
...
function initializePerformanceCharts() {
    ...
    // Create each chart
    if (contexts.equityCurve) createEquityCurve(contexts.equityCurve);
    if (contexts.winLossRatio) createWinLossChart(contexts.winLossRatio);
    if (contexts.riskReward) createRiskRewardChart(contexts.riskReward);
    if (contexts.tradeDuration) createTradeDurationChart(contexts.tradeDuration);
    if (contexts.hourlyHeatmap) createHourlyHeatmapChart(contexts.hourlyHeatmap);
    if (contexts.weeklyPerformance) createWeeklyPerformanceChart(contexts.weeklyPerformance);
    if (contexts.tradeEvaluation) createTradeEvaluationChart(contexts.tradeEvaluation);
    if (contexts.dailyPerformance) createDailyPerformanceChart(contexts.dailyPerformance);
    if (contexts.positionSize) createPositionSizeChart(contexts.positionSize);
}

// Update initializePerformanceCharts to include all charts
function initializePerformanceCharts() {
    // Destroy existing charts
    Object.values(chartInstances).forEach(chart => chart?.destroy());
    chartInstances = {};
...
// Update the initializeApplication function to include initDashboard
async function initializeApplication() {
    console.log('DOM loaded, initializing application...');
    try {
        await loadData(); // Ensure data is loaded first
        await createCharts(); // Create charts with proper async handling
    } catch (error) {
...
function createCharts() {
    ...
    for (const [chartName, canvas] of Object.entries(contexts)) {
        if (!canvas) {
            console.log(`Canvas not found for ${chartName}`);
            continue;
        }
        ...
        try {
            ...
            switch(chartName) {
                case 'equityCurve':
                    ...
                    break;
                case 'winLossRatio':
                    const winLossStats = calculateWinLossRatio();
                    chartInstances[chartName] = new Chart(ctx, {
                        type: 'doughnut',
                        data: {
                            labels: ['Wins', 'Losses'],
                            datasets: [{
                                data: [winLossStats.wins, winLossStats.losses],
                                backgroundColor: ['#00c853', '#ff3d57']
                            }]
                        },
                        options: {
                            responsive: true,
                    ...
                    break;
                case 'riskReward':
                    const riskRewardData = calculateRiskReward();
                    chartInstances[chartName] = new Chart(ctx, {
                        type: 'scatter',
                        data: {
                            datasets: [{
                                label: 'Risk/Reward',
                                data: riskRewardData.map(d => ({ x: d.risk, y: d.reward })),
                    ...
                    break;
                case 'tradeDuration':
                    const durationData = calculateTradeDuration();
                    chartInstances[chartName] = new Chart(ctx, {
                        type: 'bar',
                        data: {
                    ...
                    break;
                case 'hourlyHeatmap':
                    const hourlyStats = calculateHourlyPerformance();
                    chartInstances[chartName] = new Chart(ctx, {
                        type: 'bar',
                        data: {
                            ...
                            datasets: [{
                                ...
                                backgroundColor: hourlyStats.hourlyData.map(v => 
                                    v >= 0 ? 'rgba(0, 200, 83, 0.5)' : 'rgba(255, 61, 87, 0.5)'
                                )
                            }]
                        },
                        options: commonOptions
                    ...
                    break;
                case 'weeklyPerformance':
                    chartInstances[chartName] = createWeeklyPerformanceChart(ctx);
                    break;
                case 'tradeEvaluation':
                    const tradeScores = calculateTradeScores();
                    chartInstances[chartName] = new Chart(ctx, {
                        type: 'bar',
                        data: {
                            labels: tradeScores.map((_, i) => `Trade ${i + 1}`),
                            datasets: [{
                                label: 'Trade Score',
                                data: tradeScores.map(d => d.y),
                                backgroundColor: '#00c853'
                            }]
                        },
                        options: commonOptions
